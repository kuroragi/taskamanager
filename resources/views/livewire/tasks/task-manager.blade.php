        ...
        <form wire:submit="{{ $editing ? 'update' : 'create' }}">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" wire:model="form.difficulty" min="1" max="10"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        required>
                ...
                <div>
                    ...
                    <input type="number" wire:model="form.desire" min="1" max="10"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        required>
                    @error('form.desire')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                ...
                <div>
                    ...
                    <input type="number" wire:model="form.obligation" min="1" max="10"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        required>
                    @error('form.obligation')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" wire:model="form.energy" min="1" max="10"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('form.energy')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
            ...
            <div class="flex space-x-3 mt-6">
                ...
                @if ($editing)
                    <button type="button" wire:click="archive({{ $editing->id }})"
                    ...
                    <button type="button" wire:click="delete({{ $editing->id }})"
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($tasks as $task)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="text-sm font-bold text-indigo-600">{{ number_format($task->priority_score, 2) }}</span>
                        </td>
